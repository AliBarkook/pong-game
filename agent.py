...
class Agent():
    def __init__(self,possible_actions,starting_mem_len,max_mem_len,starting_epsilon,learn_rate, starting_lives = 5, debug = False):
        ...
        self.model_target = tf.keras.clone_model(self.model)
    ...
    def _build_model(self):
        ...
        model.add(tf.keras.Input((84,84,4)))
        model.add(tf.keras.Conv2D(filters = 32,kernel_size = (8,8),strides = 4,data_format="channels_last", activation = 'relu',kernel_initializer = tf.keras.initializers.VarianceScaling(scale=2)))
        model.add(tf.keras.Conv2D(filters = 64,kernel_size = (4,4),strides = 2,data_format="channels_last", activation = 'relu',kernel_initializer = tf.keras.initializers.VarianceScaling(scale=2)))
        model.add(tf.keras.Conv2D(filters = 64,kernel_size = (3,3),strides = 1,data_format="channels_last", activation = 'relu',kernel_initializer = tf.keras.initializers.VarianceScaling(scale=2)))
        model.add(tf.keras.Flatten())
        model.add(tf.keras.Dense(512,activation = 'relu', kernel_initializer = tf.keras.initializers.VarianceScaling(scale=2)))
        model.add(tf.keras.Dense(len(self.possible_actions), activation = 'linear'))
        optimizer = tf.keras.Adam(self.learn_rate)
